...
type PackmanMood = 'happy' | 'sleepy' | 'curious';

interface PackmanProps { mood: PackmanMood; }

const mouthPath: Record<PackmanMood, string> = {
...
const eyeY: Record<PackmanMood, number> = {
  happy: 42,
  sleepy: 48,
  curious: 38,
};
...
const Packman: FC<PackmanProps> = ({ mood }) => (
  <svg width="120" height="120" viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Packman pause pet">
    <defs>
      <radialGradient id="packmanGlow" cx="34%" cy="28%" r="72%">
        <stop offset="0%" stopColor="#FFF7A8" />
        <stop offset="48%" stopColor="#FFD83D" />
        <stop offset="100%" stopColor="#F4A900" />
      </radialGradient>
      <filter id="packmanShadow" x="-20%" y="-20%" width="140%" height="150%">
        <feDropShadow dx="0" dy="8" stdDeviation="6" floodColor="#F4A900" floodOpacity="0.26" />
      </filter>
    </defs>

    <ellipse cx="78" cy="142" rx="48" ry="8" fill="rgba(244,169,0,0.24)" />
    <g filter="url(#packmanShadow)">
      <path d={mouthPath[mood]} fill="url(#packmanGlow)" stroke="#FFF0A0" strokeWidth="4" strokeLinejoin="round" />
